...
This document describes two generalized schemas used across Causal_Web log files. `frame` is the primary time index; some tools retain a legacy `tick` for backward compatibility. IPC payloads use float32 where applicable to reduce message size.

## Diagnostic Per-Frame Schema

Diagnostic logs capture granular flow, emission decisions and failures. Modern
entries carry a `frame` field with an optional legacy `tick`. Despite varying
payloads they all share a common wrapper generated by `GenericLogEntry`.

```
log_id: str            # unique entry id
frame: int             # sequential frame number
tick: int?             # legacy tick identifier
